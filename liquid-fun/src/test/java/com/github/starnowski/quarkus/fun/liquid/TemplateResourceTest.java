...
class TemplateResourceTest {
    ...
    public void shouldGenerateMapRequestBasedOnTemplateJson(String requestFile, String templateFile, String expectedContentFile) throws IOException, JSONException {
        ExtractableResponse<Response> response = given()
                .body(Files.readString(Paths.get(new File(getClass().getClassLoader().getResource(requestFile).getFile()).getPath())))
        ...
        String expectedJson = Files.readString(Paths.get(new File(getClass().getClassLoader().getResource(expectedContentFile).getFile()).getPath()));
        JSONAssert.assertEquals(expectedJson, responsePayload, false);
